...
<div class="flex flex-wrap items-center gap-3 mb-6">
    <a href="<?= base_url('admin/pesanan') ?>" class="text-brownlite hover:underline">← Kembali</a>
    <h1 class="font-serif text-2xl font-bold text-browndark dark:text-brownlite">Detail Pesanan #<?= $pesanan['id'] ?></h1>
    <a href="<?= base_url('admin/pesanan/struk/' . $pesanan['id']) ?>" target="_blank"
       class="ml-auto px-4 py-2 bg-browndark text-white rounded-full text-sm hover:bg-brownlite transition">
        🧾 Cetak Struk
    </a>
</div>
    ...
    <div class="space-y-6">
        <div class="bg-white dark:bg-stone-800 rounded-2xl shadow-sm p-5 sm:p-6">
            ...
            <div class="space-y-2 text-sm">
                <p><span class="text-gray-500">Pemesan:</span> <?= esc($pesanan['nama_user'] ?? '-') ?></p>
                <p><span class="text-gray-500">Email:</span> <?= esc($pesanan['email'] ?? '-') ?></p>
                <p><span class="text-gray-500">Status:</span> <span class="px-2 py-1 rounded-full text-xs <?= badge_status($pesanan['status']) ?>"><?= $pesanan['status'] ?></span></p>
                <?php if (! empty($pesanan['nomor_telepon'])): ?>
                    <p>
                        <span class="text-gray-500">Telepon:</span>
                        <a href="tel:<?= esc($pesanan['nomor_telepon']) ?>" class="text-brownlite font-medium hover:underline">
                            📞 <?= esc($pesanan['nomor_telepon']) ?>
                        </a>
                    </p>
                <?php endif; ?>
            ...
            <form action="<?= base_url('admin/pesanan/status/' . $pesanan['id']) ?>" method="post" class="mt-5">
                <?= csrf_field() ?>
        ...
        <div class="bg-white dark:bg-stone-800 rounded-2xl shadow-sm p-5 sm:p-6">
            ...
            <div class="text-sm space-y-2">
                <div class="flex justify-between">
                    ...
                    <span class="font-medium"><?= esc($pesanan['metode_bayar'] ?? '-') ?></span>
                </div>
                <div class="flex justify-between">
            ...
            <form action="<?= base_url('admin/pesanan/bayar/' . $pesanan['id']) ?>" method="post" class="mt-4">
                <?= csrf_field() ?>
                ...
                <button class="w-full py-2 rounded-lg text-white transition <?= $sudah ? 'bg-yellow-600 hover:bg-yellow-700' : 'bg-green-600 hover:bg-green-700' ?>">
                    <?= $sudah ? 'Tandai Belum Dibayar' : '✓ Tandai Sudah Dibayar' ?>
                </button>
            </form>
        </div>
    </div>
